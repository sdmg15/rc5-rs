...
pub trait Rc5Trait {
	...
	fn mixin_secret_key(ctrl_block: &ControlBlock, expanded_kt: &mut Vec<Self::Word>, sec_key: &mut Vec<Self::Word>);
		...
		match hex::decode(s) {
			...
			Ok(hex_str) => {
				...
				if secret_key_len as usize != secret_key.len() {
					return Err(Rc5Error::KeyLengthMismatch{expected: secret_key_len as usize, got: secret_key.len() })
				}
				...
				Ok(Self { version, word_size, rounds, secret_key_len, secret_key: secret_key.to_vec() })
			}
		}
	}
}
...
impl Rc5Trait for RC5 {
	...
	fn convert_secret_key_from_bytes_to_word(ctrl_block: &ControlBlock) -> Vec<Self::Word> {
		...
		(0..ctrl_block.secret_key_len).rev().for_each(|i| {
			...
			sec_key_as_words[u as usize] = (sec_key_as_words[u as usize] << 8) + ctrl_block.secret_key[i as usize] as u32;
		});
		sec_key_as_words
	}
	...
	fn mixin_secret_key(ctrl_block: &ControlBlock, expanded_kt: &mut Vec<Self::Word>, sec_key: &mut Vec<Self::Word>) {
		...
		(0..3*expanded_kt_len).for_each(|_| {
			let mixed_key_value = expanded_kt[i as usize].wrapping_add(mixed_a.wrapping_add(mixed_b));
			let mixed_key_value = mixed_key_value.rotate_left(3);

			expanded_kt[i as usize] = mixed_key_value;
			mixed_a = mixed_key_value;

			let mixed_secret_value = sec_key[j as usize].wrapping_add(mixed_a.wrapping_add(mixed_b));
	...
	fn encrypt(&self, pt: &[Self::Word; 2]) -> [Self::Word; 2] {
		...
		(1..=self.control_block.rounds).for_each(|i| {
			encrypted_a = (encrypted_a^encrypted_b).rotate_left(encrypted_b)
												   .wrapping_add(kt[2 * i as usize]);

			encrypted_b = (encrypted_b^encrypted_a).rotate_left(encrypted_a)
	...
	fn decrypt(&self, ct: &[Self::Word; 2]) -> [Self::Word; 2] {
		...
		(1..=rounds).rev().for_each(|i| {
			encrypted_b = encrypted_b.wrapping_sub(kt[(2 * i + 1) as usize])
									 .rotate_right(encrypted_a) ^ encrypted_a;

			encrypted_a = encrypted_a.wrapping_sub(kt[(2 * i) as usize])
									 .rotate_right(encrypted_b) ^ encrypted_b;
		});
...
use crate::helper::{word_as_str, to_words};
	...
	fn test_encryption2() {
		...
		let secret_key = vec![0x91, 0x5F, 0x46, 0x19, 0xBE, 0x41, 0xB2, 0x51, 0x63, 0x55, 0xA5, 0x01, 0x10, 0xA9, 0xCE, 0x91];
	...
	fn test_encryption3() {
		let mut ctrl = ControlBlock::default();
		let secret_key = vec![0x78, 0x33, 0x48, 0xE7, 0x5A, 0xEB, 0x0F, 0x2F, 0xD7, 0xB1, 0x69, 0xBB, 0x8D, 0xC1, 0x67, 0x87];
		ctrl.set_secret_key(secret_key);
	...
	fn test_encryption4() {
		let mut ctrl = ControlBlock::default();
		let secret_key = vec![0xDC, 0x49, 0xDB, 0x13, 0x75, 0xA5, 0x58, 0x4F, 0x64, 0x85, 0xB4, 0x13, 0xB5, 0xF1, 0x2B, 0xAF];
		ctrl.set_secret_key(secret_key);
	...
	fn test_encryption5() {
		let mut ctrl = ControlBlock::default();
		let secret_key = vec![0x52, 0x69, 0xF1, 0x49, 0xD4, 0x1B, 0xA0, 0x15, 0x24, 0x97, 0x57, 0x4D, 0x7F, 0x15, 0x31, 0x25];
		ctrl.set_secret_key(secret_key);
